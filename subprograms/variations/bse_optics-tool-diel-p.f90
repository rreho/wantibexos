...
!ifort -mcmodel=large bse_optics-tool-diel.f90 -o bse_opt-tool-diel-n3.x -mkl -qopenmp
...
program main
	...
	OPEN(UNIT=301, FILE=trim(outputfolder)//"bse_opt_diel.dat",STATUS='unknown', IOSTAT=erro)
    	if (erro/=0) stop "Erro na abertura do arquivo de saida bse diel xx yy zz xy xz yz"
	...
	OPEN(UNIT=2077, FILE= trim(calcparms)//"bse_opt_tool_calc.dat",STATUS='unknown', IOSTAT=erro)
	...
	do i=1,dimbse
		...
		call optrpabse(eigv(stt(i,4),stt(i,2)),vector(stt(i,4),stt(i,2),:),&
...
!	      RWORK, LRWORK, IWORK, LIWORK,INFO )
 !     	IF( INFO.GT. 0 ) THEN
  ...
  !    	END IF   
	...
	call dielbsep(nthreads,dimbse,hbse,hrx,hrx,actxx)
	call dielbsep(nthreads,dimbse,hbse,hry,hry,actyy)
	call dielbsep(nthreads,dimbse,hbse,hrz,hrz,actzz)
